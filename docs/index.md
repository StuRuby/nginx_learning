# 目录

- [macos 源码编译nginx](https://sturuby.github.io/nginx_learning/building_nginx_from_sources_on_osx)
- [nginx配置文件的通用语法](https://sturuby.github.io/nginx_learning/nginx_grammar)
- [nginx命令行命令](https://sturuby.github.io/nginx_learning/nginx_命令行)
- [基于OpenResty用Lua语言实现简单服务](https://sturuby.github.io/nginx_learning/基于OpenResty用Lua语言实现简单服务)
- [用GoAccess实现可视化并实时监控access日志](https://sturuby.github.io/nginx_learning/用GoAccess实现可视化并实时监控access日志)
- [用nginx搭建一个可用的静态资源web服务器](https://sturuby.github.io/nginx_learning/用nginx搭建一个可用的静态资源web服务器)
- [用nginx搭建一个具备缓存功能的反向代理服务](https://sturuby.github.io/nginx_learning/用nginx搭建一个具备缓存功能的反向代理服务)
- [nginx配置指南](https://sturuby.github.io/nginx_learning/nginx配置指南)
- [Nginx反向代理](https://sturuby.github.io/nginx_learning/Nginx作为反向代理)
